...
UnityLight CreateLight(float3 normal, fixed detailShadowMap)
{
    ...
    light.color = saturate(_LightColor0.rgb * lerp(1, poiLight.attenuation, float(0.3)) * detailShadowMap);
...
float FadeShadows(float attenuation)
{
            ...
            attenuation = lerp(1, poiLight.attenuation, float(0.3));
        #endif
...
void ApplySubtractiveLighting(inout UnityIndirect indirectLight)
{
        ...
        poiLight.attenuation = FadeShadows(lerp(1, poiLight.attenuation, float(0.3)));
...
{
    return UNITY_BRDF_PBS(1, 0, 0, float(0.3), poiMesh.normals[1], poiCam.viewDir, CreateLight(poiMesh.normals[1], detailShadowMap), CreateIndirectLight(poiMesh.normals[1])).xyz;
}
void calculateBasePassLightMaps()
{
        ...
        float bw_directLighting = (((poiLight.nDotL * 0.5 + 0.5) * bw_lightColor * lerp(1, poiLight.attenuation, float(0.3))) + dot(ShadeSH9Normal(poiMesh.normals[1]), grayscale_vector));
        ...
        {
            poiLight.directLighting = (_LightColor0.rgb) * saturate(RTWrapFunc(poiLight.nDotL, float(0), float(0))) * detailShadow * lerp(1, poiLight.attenuation, float(0.3));
...
float3 calculateNonImportantLighting(float attenuation, float attenuationDotNL, float3 albedo, float3 lightColor, half dotNL, half correctedDotNL)
{
    ...
    {
        return lerp(lightColor * attenuation, lightColor * float(1) * attenuation, smoothstep(float(0), float(0.5), dotNL)) * detailShadow;
    }
...
float3 calculateFinalLighting(inout float3 albedo, float4 finalColor)
{
    float3 finalLighting = 1;
        ...
        else if (float(1) == 1) // Toon
        {
                ...
                finalLighting = lerp(poiLight.color * max(poiLight.additiveShadow, float(1)), poiLight.color * float(1), smoothstep(float(0), float(0.5), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation * detailShadow;
            #else
                finalLighting = lerp(poiLight.color * max(poiLight.attenuation, float(1)), poiLight.color * float(1), smoothstep(float(0), float(0.5), 1 - (.5 * poiLight.nDotL + .5))) * detailShadow;
            #endif
        }
        else //if(float(1) == 2) // Wrapped
        {
            ...
            if (float(0) == 1) // Math Gradient
            poiLight.rampedLightMap = lerp(float4(1,1,1,1), float3(1, 1, 1), saturate(1 - smoothstep(float(0) - .000001, float(0.5), 1 - uv)));
            float shadowatten = max(poiLight.additiveShadow, float(1));
